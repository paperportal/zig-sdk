...
pub fn addPortalApp(b: *std.Build, opts: PortalAppOptions) PortalApp {
    ...
    const required_exports = [_][]const u8{ "portalContractVersion", "portalAlloc", "portalFree", "portalMicroTaskStep", "portalGesture" };
    ...
    const exe = b.addExecutable(.{ .name = opts.exe_name, .root_module = root_mod });
    ...
    const sdk_dep = blk: {
        if (opts.local_sdk_path) |path| {
            if (dirExists(b, path)) {
                ...
                break :blk (b.lazyDependency(local_name, .{}) orelse @panic("local sdk dependency missing"));
            }
        }
        break :blk b.dependency(opts.sdk_dependency_name, .{});
    };
