...
/// Framebuffer display precision / color depth mode.
///
/// This is separate from `display.epd.setMode()`, which controls the EPD refresh waveform.
pub const DisplayMode = enum(i32) {
    ...
    gray16 = 2,
    /// 8bpp (256 grayscale levels). Note: FastEPD does not support this mode.
    gray256 = 3,
};

/// Set the framebuffer display mode (color depth / grayscale precision).
///
/// This does not clear the backbuffer and does not trigger a refresh; callers should redraw.
pub fn setDisplayMode(mode: DisplayMode) Error!void {
    try errors.check(ffi.setDisplayMode(@intFromEnum(mode)));
}
...
pub const image = struct {
    /// Firmware-provided embedded icons.
    ...
    /// Draw an embedded firmware icon with the top-left corner at (`x`, `y`).
    pub fn drawIcon(x: i32, y: i32, which: Icon) Error!void {
        try errors.check(ffi.drawIcon(x, y, @intFromEnum(which)));
    }
    ...
    pub fn drawXth(xth_bytes: []const u8, fast: bool) Error!void {
        try errors.check(ffi.drawXth(xth_bytes.ptr, xth_bytes.len, fast));
    }

    /// Decode and draw an XTG (XTEINK, monochrome 1bpp) image, centered on the display.
    ///
    /// Empty input is a no-op. The firmware rejects inputs larger than 1 MiB.
    pub fn drawXtg(xtg_bytes: []const u8, fast: bool) Error!void {
        try errors.check(ffi.drawXtg(xtg_bytes.ptr, xtg_bytes.len, fast));
    }
...
pub const vlw = struct {
    ...
    pub fn useSystem(font: SystemFont, font_size: i32) Error!void {
        try errors.check(ffi.vlwUseSystem(@intFromEnum(font), font_size));
    }
