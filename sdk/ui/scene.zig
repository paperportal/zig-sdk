...
///
/// This module is pure Zig (no host FFI).
///
/// Typical app wiring uses the SDK-managed scene stack via `sdk.ui.scene.*`:
/// - `sdk.ui.scene.set(...)` to set the root scene.
/// - Gesture events are delivered via the SDK-exported `portalGesture` callback.
...
pub const SceneStack = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, max_scenes: usize) SceneStack {
        return .{
            .allocator = allocator,
            .ctx = .{
                .allocator = allocator,
    ...
    /// Clear the whole stack and set a new root scene.
    ...
    pub fn replace(self: *SceneStack, scene: Scene) anyerror!void {
...
pub const Navigator = struct {
    ...
    pub fn replace(self: *Navigator, scene: Scene) anyerror!void {
        try self.stack.replace(scene);
    }
...
test "SceneStack basic navigation order" {
    ...
    var stack = SceneStack.init(std.testing.allocator, 8);
    defer stack.deinit();

    // set (root): enter then draw
    try stack.set(Scene.from(T, &s1));
    ...
    // replace (top): exit old top, enter new, draw new
    try stack.replace(Scene.from(T, &s3));
...
test "SceneStack set clears and draws new root" {
    var log: std.ArrayList(u8) = .empty;
    defer log.deinit(std.testing.allocator);
    ...
    try stack.set(Scene.from(T, &a));
    try stack.push(Scene.from(T, &b));
    try std.testing.expect(stack.len() == 2);

    // set: best-effort exits (top to bottom), then enter+draw new root
    try stack.set(Scene.from(T, &c));
