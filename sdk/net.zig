...
pub const WifiMode = enum(i32) {
    /// Wi-Fi is stopped/disabled (`WIFI_MODE_NULL`).
    off = 0,
    /// Station mode (`WIFI_MODE_STA`).
    sta = 1,
    /// SoftAP mode (`WIFI_MODE_AP`).
    ap = 2,
...
pub fn wifiGetMode() Error!WifiMode {
    const raw = try errors.checkI32(ffi.wifiGetMode());
    return std.enums.fromInt(WifiMode, raw) orelse Error.Unknown;
}
