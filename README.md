...
The SDK provides `sdk.microtask` for host-scheduled cooperative work:

    const sdk = @import("paper_portal_sdk");
    const microtask = sdk.microtask;

    const Heartbeat = struct {
        pub fn step(self: *Heartbeat, now_ms: u32) anyerror!microtask.Action {
            _ = self;
            _ = now_ms;
    ...
    const handle = try microtask.start(microtask.Task.from(Heartbeat, &heartbeat), 0, 500);
    defer microtask.cancel(handle) catch {};

`portalMicroTaskStep` is exported by the SDK automatically. Apps should not define that export manually.

See `docs/microtasks-module-guide.md` for the full module guide.
...
`portalGesture` callback (exported by the SDK automatically).
